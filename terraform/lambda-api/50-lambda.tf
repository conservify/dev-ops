...
resource "aws_lambda_permission" "lambda_api_gateway_permission" {
  ...
  source_arn = "arn:aws:execute-api:${var.region}:${var.owner_id}:${aws_api_gateway_rest_api.fk-api-incoming.id}/*/${aws_api_gateway_method.fk-api-incoming-messages-enqueue-method.http_method}${aws_api_gateway_resource.fk-api-incoming-messages.path}"
}
